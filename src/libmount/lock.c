 ...
 */
mlock *mnt_new_lock(const char *lockfile, pid_t id)
{
	mlock *ml = calloc(1, sizeof(struct _mlock));
 ...
 */
const char *mnt_lock_get_lockfile(mlock *ml)
{
	if (!ml)
		return NULL;
	if (ml->lockfile)
 ...
 */
const char *mnt_lock_get_linkfile(mlock *ml)
{
	if (!ml)
		return NULL;
 ...
 */
static int mnt_wait_lock(mlock *ml, struct flock *fl, time_t maxtime)
{
	struct timeval now;
...
/**
 * mnt_lock:
 * @ml: pointer to mlock instance
 * @errbuf: buffer for error message or NULL
 * @errsz: size of error buffer
 *
 * Creates lock file (e.g. /etc/mtab~). Note that this function uses
 * alarm().
 *
 * Your application has to always call mnt_unlock() before exit.
 *
 * Locking scheme:
 *
